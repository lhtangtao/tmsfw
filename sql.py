...
def create_table():
    ...
    cur = conn.cursor()
    try:
        sql_script = 'CREATE TABLE new_house' + current_data + " (`ID` integer(100) NOT NULL AUTO_INCREMENT COMMENT '唯一主键', `location` varchar(300) CHARACTER SET ascii NOT NULL DEFAULT '' COMMENT '所在行政区域',  `village` varchar(30) NOT NULL DEFAULT '' COMMENT '小区名字', `other_name` varchar(30) NULL DEFAULT '' COMMENT '小区别名',`building` varchar(300) NULL DEFAULT '' COMMENT '楼栋', `room` varchar(30) NULL COMMENT '房号',`area` double NULL COMMENT '建筑面积', `inner_area` double NULL COMMENT '套内面积',`rate` double NULL COMMENT '得房率',  `billet_unit_price` double NULL COMMENT '毛坯单价',  `decoration_price` double NULL COMMENT '装修价',  `total_price` double NULL COMMENT '总价',  `operation` varchar(30) NULL COMMENT '操作', `url_address` varchar(100) NULL COMMENT '具体链接',  PRIMARY KEY (`ID`)) ENGINE=InnoDB AUTO_INCREMENT=15 DEFAULT CHARSET=utf8;"
        ...
        cur.execute(sql_script)
        sql_script = "ALTER TABLE new_house" + current_data + " MODIFY COLUMN `location` varchar(1000) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '' COMMENT '所在具体未知' AFTER `ID`,ADD COLUMN `administrative` varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL DEFAULT '' COMMENT '所在行政区域' AFTER `location`;"
        cur.execute(sql_script)
        x = True
